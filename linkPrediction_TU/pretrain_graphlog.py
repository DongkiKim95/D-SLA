...
class MaskNode:
    ...
    def __call__(self, data):
        ...
        # masking token
        masked_data.x[mask_idx] = self.num_node_type
        masked_data.masked_node_index = mask_idx
...
def intra_NCE_loss(node_reps, node_modify_reps, batch, tau=0.1, epsilon=1e-6):
    ...
    exp_sim_all = torch.index_select(exp_sim_mask, 1, batch.masked_node_index)
    exp_sim_positive = torch.index_select(exp_sim_all, 0, batch.masked_node_index)
    positive_ratio = exp_sim_positive.sum(0) / (exp_sim_all.sum(0) + epsilon)

    NCE_loss = -torch.log(positive_ratio).sum() / batch.masked_node_index.shape[0]
    mask_select = torch.index_select(mask, 1, batch.masked_node_index)
...
def init_proto_lowest(args, model, proj, loader, device, num_iter = 5):
    ...
    for iter in range(num_iter):
        for step, batch in enumerate(tqdm(loader, desc="Iteration")):
            ...
            node_reps = model(batch.x, batch.edge_index)
...
def pretrain(args, model, proj, loader, optimizer, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        batch, batch_modify = batch
        batch, batch_modify = batch.to(device), batch_modify.to(device)

        # get node and graph representations
        node_reps = model(batch.x, batch.edge_index)
        node_modify_reps = model(batch_modify.x, batch_modify.edge_index)
        graph_reps = pool_func(node_reps, batch.batch, mode=args.graph_pooling)
        ...
        total_intra_cnt += batch_modify.masked_node_index.shape[0]
...
def main():
    ...
    model = GNN(args.num_layer, args.emb_dim, num_node_type, JK=args.JK, drop_ratio=args.dropout_ratio,
