...
class Augmentation():
    def __init__(self, aug_ratio, num_node_type):
        self.aug_ratio = aug_ratio
        self.num_node_type = num_node_type

        self.augmentations = [self.node_drop, self.subgraph, self.edge_pert, self.attr_mask]

    def __call__(self, data):
        data1 = data.clone()
        data2 = data.clone()

        aug1 = np.random.choice(range(len(self.augmentations)), 1)[0]
        aug2 = np.random.choice(range(len(self.augmentations)), 1)[0]
        data1 = self.augmentations[aug1](data1, self.aug_ratio)
        data2 = self.augmentations[aug2](data2, self.aug_ratio)
...
def main():
    ...
    dataset.transform = Augmentation(args.aug_ratio, num_node_type)
    print(dataset)
    ...
    for epoch in range(1, args.epochs+1):
        pretrain_loss = train(args, loader, model, optimizer, device)
