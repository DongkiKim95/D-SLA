...
def main():
    ...
    parser.add_argument('--dataset', type=str, default = 'tox21',
                         choices=['bbbp', 'clintox', 'muv', 'hiv', 'bace', 'sider', 'tox21', 'toxcast'])
    ...
    with open(os.path.join(result_dir,'result.txt'), 'a+') as f:
        f.write(args.dataset + ' ' + str(args.runseed) + ' ' + str(np.array(val_acc_list).max()) + ' ' + str(np.array(test_acc_list)[np.array(val_acc_list).argmax()]) + ' ' + str(np.array(val_acc_list)[-1]) + ' ' + str(np.array(test_acc_list)[-1]))
        f.write('\n')
