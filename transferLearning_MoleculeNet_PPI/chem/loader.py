...
class MoleculeDataset(InMemoryDataset):
    def __init__(self,
        ...
        if self.smi:
            with open(os.path.join(self.root, 'processed', 'smiles.csv'), 'r') as f:
                self.smiles = f.readlines()
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for i in tqdm(range(len(smiles_list)), desc='ZINC Processing'):
                s = smiles_list[i]
            ...
            print('processing')
            for i in tqdm(range(len(rdkit_mol_objs)), desc='ChEMBL Processing'):
            ...
            for i in tqdm(range(len(smiles_list)), desc='Tox21 Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                ## convert aromatic bonds to double bonds
                #Chem.SanitizeMol(rdkit_mol,
                                 #sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
            ...
            for i in tqdm(range(len(smiles_list)), desc='HIV Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
                # Chem.SanitizeMol(rdkit_mol,
                #                  sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
            ...
            for i in tqdm(range(len(smiles_list)), desc='BACE Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
                # Chem.SanitizeMol(rdkit_mol,
                #                  sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
            ...
            for i in tqdm(range(len(smiles_list)), desc='BBBP Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                if rdkit_mol != None:
                    # # convert aromatic bonds to double bonds
                    # Chem.SanitizeMol(rdkit_mol,
            ...
            for i in tqdm(range(len(smiles_list)), desc='ClinTox Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                if rdkit_mol != None:
                    # # convert aromatic bonds to double bonds
                    # Chem.SanitizeMol(rdkit_mol,
            ...
            for i in tqdm(range(len(smiles_list)), desc='ESOL Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
                # Chem.SanitizeMol(rdkit_mol,
                #                  sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
            ...
            for i in tqdm(range(len(smiles_list)), desc='FreeSolv Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
                # Chem.SanitizeMol(rdkit_mol,
                #                  sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
            ...
            for i in tqdm(range(len(smiles_list)), desc='Lipophilicity Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
                # Chem.SanitizeMol(rdkit_mol,
                #                  sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
            ...
            for i in tqdm(range(len(smiles_list)), desc='MUV Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
                # Chem.SanitizeMol(rdkit_mol,
                #                  sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
            ...
            for i in tqdm(range(len(smiles_list)), desc='PCBA Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
                # Chem.SanitizeMol(rdkit_mol,
                #                  sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
            ...
            downstream_inchi = set(pd.read_csv(os.path.join(self.root,
                                               ...
                                               sep=',', header=None)[0])
            for i in tqdm(range(len(smiles_list)), desc='PCBA_Pretrain Processing'):
        ...
        # elif self.dataset == ''
            ...
            for i in tqdm(range(len(smiles_list)), desc='SIDER Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                # # convert aromatic bonds to double bonds
                # Chem.SanitizeMol(rdkit_mol,
                #                  sanitizeOps=Chem.SanitizeFlags.SANITIZE_KEKULIZE)
            ...
            for i in tqdm(range(len(smiles_list)), desc='ToxCast Processing'):
                rdkit_mol = rdkit_mol_objs[i]
                if rdkit_mol != None:
                    # # convert aromatic bonds to double bonds
                    # Chem.SanitizeMol(rdkit_mol,
            ...
            for i in tqdm(range(len(smiles_list)), desc='PTC-MR Processing'):
                s = smiles_list[i]
                rdkit_mol = AllChem.MolFromSmiles(s)
                if rdkit_mol != None:  # ignore invalid mol objects
                    # # convert aromatic bonds to double bonds
            ...
            for i in tqdm(range(len(smiles_list)), desc='MUTAG Processing'):
                s = smiles_list[i]
                rdkit_mol = AllChem.MolFromSmiles(s)
                if rdkit_mol != None:  # ignore invalid mol objects
                    # # convert aromatic bonds to double bonds
...
class MoleculeDataset_candidate(MoleculeDataset):
    def __init__(self,
                 root,
                 edge_pert_strength,
                 mask_strength,
        ...
        super(MoleculeDataset_candidate, self).__init__(root, transform=transform, \
                    pre_transform=pre_transform, pre_filter=pre_filter, dataset=dataset, empty=empty)
    ...
    def subgraph_edge_pert(self, data, subset, sub_edge_index, edge_mask, i):
        ...
        # edge attribute generation: strictly follow the distribution of edge attributes in pre-training dataset
        edge_attr_add_1 = torch.tensor( np.random.choice([0,1,2,3], size=(pert_num,), \
                                        p=[0.375, 0.296, 0.011, 0.318]))
        edge_attr_add_1 = torch.cat([edge_attr_add_1]*2, dim=0)


        single_bond_idx = (edge_attr_add_1 == 0).nonzero(as_tuple=False).squeeze(-1)
        aromatic_bond_idx = (edge_attr_add_1 == 3).nonzero(as_tuple=False).squeeze(-1)

        single_bond_dir = torch.tensor( np.random.choice([0,1,2], size=(single_bond_idx.size(0), ), \
                                        p=[0.8723, 0.0867, 0.0410]))
        aromatic_bond_dir = torch.tensor( np.random.choice([0,1,2], size=(aromatic_bond_idx.size(0),), \
                                        p=[0.9893, 0.0092, 0.0015]))
                            
        edge_attr_add_2 = torch.zeros((edge_index_add.shape[1],), dtype = torch.long)
        edge_attr_add_2.scatter_(0, single_bond_idx, single_bond_dir)
        edge_attr_add_2.scatter_(0, aromatic_bond_idx, aromatic_bond_dir)

        edge_attr_add_1 = edge_attr_add_1.view(-1,1)
        edge_attr_add_2 = edge_attr_add_2.view(-1, 1)
        ...
        return data, pert_num

    def attr_mask(self, data, subset=None):
        node_num = len(subset)
        mask_num = int(node_num * self.mask_strength)

        idx_mask = np.random.choice(subset, mask_num, replace=False)

        token = torch.tensor([119,0], dtype=torch.long)
    ...
    def get(self, idx):
        ...
        for i in range(self.num_candidates):
            ...
            if self.return_strong_pert and i == self.num_candidates - 1:
                ...
                candidates[i], mask_index, answer = self.attr_mask(candidates[i], all_set)

                candidates[i], dist = self.subgraph_edge_pert(candidates[i], all_set, edge_index, edge_mask, 4)
                candidates[i].dist = dist
                break
                
            # Masking node attributes
            candidates[i], mask_index, answer = self.attr_mask(candidates[i], subset)
